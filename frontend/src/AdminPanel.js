    ...
    <main style={{ maxWidth: '800px', margin: 'auto', padding: '2rem' }}>
      ...
      <SectionCard title="📊 Metrics">
        {!metrics ? (
          ...
          <ul>
            <li><strong>Hostname:</strong> {metrics.hostname || 'N/A'}</li>
            <li><strong>Platform:</strong> {metrics.platform || 'N/A'}</li>
            <li><strong>Platform Version:</strong> {metrics.platform_version || 'N/A'}</li>
            <li><strong>Uptime:</strong> {metrics.uptime_secs != null ? `${(metrics.uptime_secs / 3600).toFixed(1)} hrs` : 'N/A'}</li>
            <li><strong>CPU Count:</strong> {metrics.cpu?.cores ?? 'N/A'}</li>
            <li><strong>CPU Load %:</strong> {metrics.cpu?.percent ?? 'N/A'}%</li>
            <li><strong>Memory Used %:</strong> {metrics.memory?.percent ?? 'N/A'}%</li>
            <li><strong>Disk Used %:</strong> {metrics.disk?.percent ?? 'N/A'}%</li>
            <li><strong>Timestamp:</strong> {new Date(metrics.timestamp).toLocaleString()}</li>
          </ul>
        )}
      </SectionCard>
      </div>
        ...
        <ul>
          {users.map((user) => (
            <li key={user.username}>
              ...
              <button onClick={() => handleDelete(user.username)} style={{ fontSize: '0.8rem', backgroundColor: '#eee', border: '1px solid #ccc', padding: '2px 6px', marginLeft: '0.5rem' }} >
  Delete
</button>
